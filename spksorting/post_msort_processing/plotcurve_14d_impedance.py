...
def custom_curation(animal_name, animal_dict):
    ...
    # elif animal_name.lower()=="benmouse1":
    #     animal_dict = remove_datapoints(animal_dict, )
    return animal_dict
...
if __name__ == "__main__":
    ...
    ax.set_xlim([-0.25, N_DAYS])
    ax.set_xticks(np.arange(N_DAYS))
    ax.set_xticklabels(np.arange(N_DAYS))
    ax.set_xlabel("Day")
    ax.set_ylabel("Impedance ($k\\Omega$)")
    plt.savefig(os.path.join(PLOTFOLDER, "impedances_daily_all.png"))
    plt.savefig(os.path.join(PLOTFOLDER, "impedances_daily_all.eps"))
    plt.tight_layout()
    plt.show()
    ...
    for group_name, animal_group in zip(group_names, animal_groups):
        ...
        ax.set_xlabel("Day")
        ax.set_ylabel("Impedance ($k\\Omega$)")
        plt.savefig(os.path.join(PLOTFOLDER, "impedances_daily_%s.png"%(group_name)))
        plt.savefig(os.path.join(PLOTFOLDER, "impedances_daily_%s.eps"%(group_name)))
        plt.tight_layout()
        plt.show()